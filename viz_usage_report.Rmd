...
viz_users %>% select(name,type,int_count) %>% head() %>% 
...
Interpretations here does not include user comments.
...
There are `r length(unique(dash_stats$dash_name))` total dashboards containing `r length(unique(viz_users_dash_merged$id))` total visualization objects.
...
-   Dashboards -- Comparison (views, users, updatedvizes), over time, average views by hour of each day, distinct user-days viewing each dashboard
